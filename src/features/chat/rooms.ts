...
/**
 * ツーショットチャット（/chat/2shot/）の部屋 ID。この部屋だけは通常のチャット画面ではなく
 * features/two-shot-chat の画面を出し、会話を `chats` に書かない（.kiro/specs/two-shot-chat）。
 * ID・カテゴリ・紹介文はほかの部屋と同じくこのファイルで持ち、トップのリンクやサイトマップはそのまま使う。
 */
...
const ROOM_DESCRIPTIONS: Record<RoomId, string> = {
  ...
  // ツーショットチャットは会話を公開しない（.kiro/specs/two-shot-chat Requirement 15.4 / 15.5）
  '2shot':
    '昔ながらの2ショットチャットの再現です。会話は入室中の2人にだけ見え、後から来た人には見せません。通報対応用に控えを30日保存し（削除は最大1時間遅れ）、通報時だけ管理者が確認します。個人情報や出会い目的の書き込みは禁止です。',
