...
| Realtime の INSERT   | 確定行に uuid で合流（二分探索で挿入位置だけを探す）。取得中は、保存の応答や clear の書き換えと同じ列に発生順で記録する                          |
| 取得の完了           | 取得結果を正とし（拡張なら取得開始時の行と合流）、取得中に記録した新着と書き換えを発生順に適用し直す。clear の後に届いた同じ名前の発言は消さない |
