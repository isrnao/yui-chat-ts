...
- [ ] 9. ドキュメントと検収（Requirement 16.3 / 18、PR8）
  - [x] 9.1 CLAUDE.md に `two-shot-chat/`、Edge Function `two-shot`、`two_shot_rooms`（ログを公開しない、service_role
        だけが書く）、入室記録・監査・保存 RPC・削除ジョブを書く
    - _Requirements: 16.3_
  - [x] 9.2 S1〜S11 の Oracle との比較の結果を PR に記録する
    - _Requirements: 18.1_
    - 2026-09-24、Chromium（ヘッドレス、倍率 1）で Oracle とストーリーを撮り、閾値 32 を超える画素の割合を出した。
      差の中身は並べた画像で確かめた（画像はリポジトリの外）

      | 状態               |      1280×800 |       375×812 | 差の中身                                                            |
      | ------------------ | ------------: | ------------: | ------------------------------------------------------------------- |
      | S1 / S2 / S3 / S4  |     8.1〜8.8% |   18.6〜19.2% | Q4(a) の注意書きの分だけ表が下がる（意図した差分）                  |
      | S5                 |         0.72% |         3.45% | ログの時刻、カーソル、注意書きの 1px のずれ                         |
      | S6 / S7            | 2.46% / 1.63% | 7.78% / 5.72% | 新しい Guest に入室前のログを見せない（D18・R15.1）                 |
      | S8                 |         1.14% |         4.34% | 時刻・無発言監視の秒数（データ）と、Oracle の静的な上ペインのラジオ |
      | S9a / S9b          | 1.75% / 1.81% | 6.08% / 6.26% | D18 の入室前のログの消去と、管制者の画面クリア（Q5）                |
      | S9c                |         0.68% |         3.38% | ログの時刻、カーソル、注意書きの 1px のずれ                         |
      | S9d                |         1.07% |         6.68% | Oracle は S9c の画面クリアの続き（フィクスチャの順序）              |
      | S10a / S10b / S10c |   0.66〜0.77% |   3.31〜3.69% | カーソルと注意書きの 1px のずれ（Quirks の行の高さ）                |
      | S11a               |        0.000% |        0.000% | 一致                                                                |
      | S11b               |         0.09% |         0.25% | E2 の本文（D16、意図した差分）                                      |

  - [x] 9.3 requirements.md の Success Metrics を計測して記録する（チャンクの大きさ、modulePreload、API の p95）
    - _Requirements: 17.5, 18.1_
    - 意図した差分以外に Oracle と違う画面の状態: 0（上の表）
    - §5.2 / §5.3 の文言のテスト: N1〜N10 と E1〜E9 はテストあり。E10 と E11 は再実装では出さない（E11 の代わりの
      未知の操作は 400 で、`handler.test.ts` が確かめる。画面は E12）
    - 未認証の操作・拒否した入室で部屋が変わる経路: 0（`rules.test.ts` の fast-check）
    - `/chat/2shot/` の modulePreload の `vendor-supabase`: なし（先読みは TwoShotRoute と roomSeo だけ）
    - ルートのチャンク: gzip 14.9 kB（目標 15 kB 以下）
