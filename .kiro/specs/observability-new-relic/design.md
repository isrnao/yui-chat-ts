...
**修正後の確認**：Task 4 の E2E（下記）で、com_sb への投稿から `evaluation.run` までが 1 本のトレースになることを確認した。

**解決済み**：com_sb に投稿した約 37 秒後にあったもう 1 件の `/api/v1/evaluate` の呼び出し（trace `4fc831be…`）は、利用者が同じ時間帯に手動で試したものだった。

## Task 4：E2E 検証とマイルストーンの判定（2026-09-22）

**判定スクリプト**：[`scripts/verify-trace.ts`](../../../scripts/verify-trace.ts)

- NerdGraph で trace ID のスパンを取得し、`id` と `parent.id` から木を組み立てて、期待する木（`normal` / `admin`）と照合する。
- 照合する項目：ルートが 1 つだけか、ルートの親が指定した値か（`--parent`）、各階層の名前とサービス、ERROR の有無、余分なスパンの有無。
- 取り込みに 1〜2 分かかるので、最大 `--wait` 秒（既定 180 秒）まで 15 秒おきに再検索する。
- `--operation <id>` を付けると、同じ操作 ID の全試行を並べ、重複して保存されていないかも表示する。
- **スクリプト自体の確認**：修正前の com_sb のトレース（`b420251b…`）は「okiraku-api のスパンが見つからない」で不合格になり、通常の部屋のトレースは合格した。

**本番のシナリオ**

| シナリオ                        | 方法                                             | 結果                                                                                                                                                                                                                                    |
| ------------------------------- | ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 通常の部屋の保存（R8.1〜8.2）   | テスト投稿（`superbeginner`）                    | **合格**：`POST save-chat` → `db insert chats`                                                                                                                                                                                          |
| com_sb の保存と triage（R8.3）  | テスト投稿（`sampled=00` の `traceparent` 付き） | **合格**：6 スパン。save-chat（739ms）→ `db insert chats`（733ms）と `triage`（2013ms、`no_action`）→ `POST /api/v1/evaluate`（クライアント）→ okiraku-api `POST /api/v1/evaluate`（558ms）→ `evaluation.run`（540ms、トークン 381/45） |
| `sampled=00` でも記録（R8.8）   | 上の com_sb の投稿                               | **合格**：全スパンが記録され、ルートの親は送った値                                                                                                                                                                                      |
| 操作 ID での束ね（R8.7 の一部） | `--operation`                                    | **合格**：通常の部屋の投稿の操作 ID で、試行 1 回・トレース 1 本・保存 1 件                                                                                                                                                             |

- trace ID：通常 `11b828f5…`、com_sb（修正前）`b420251b…`、com_sb（修正後）`c6a64e27…`

**本番では実施していないシナリオ**：DB 保存の失敗、`/evaluate` のタイムアウト、OTLP の無効化、同時 2 件、初回失敗 → 再試行で成功

- 故障注入（`SAVE_CHAT_FAULT_INJECT`）は、設計どおり本番では無効になっている。staging のプロジェクトもない。
- そのため、これらは `deno test`（22 件）と okiraku-api の `node --test`（32 件）で確認している。本番で試すには、staging の用意か、テスト用の非公開ルームでの故障注入の許可が必要（未決事項）。

**R4.7：送信されたデータの確認**（テスト投稿 3 本、12 スパンの全属性）

- 本文（「使い方」「動作確認」）、名前（「監視テスト」）、IP（`133.*`）、UA（`curl/`）、`Bearer`、ライセンスキー（`NRAL`）は、**いずれも含まれていなかった**。
- 属性名は、設計した `chat.*` / `db.*` / `http.*` / `triage.*` / `evaluation.*` / `gen_ai.*` / `okiraku.*` と、New Relic が自動で付けるもの（`entity.*`、`newRelic.*`、`otel.library.*` など）だけだった。

**R9：データ量と性能**

- **データ量**：1 スパンあたり約 780 バイト（save-chat 14,079 バイト / 18 件、okiraku-api 7,935 バイト / 10 件）。
  - 直近 30 日の発言は 424 件（system を除くと 305 件、com_sb は 13 件）。
  - 月の見積もりは、424 件 × 約 2 スパン × 0.8KB に com_sb の分を足して、**約 1MB**。無料枠（100GB）の 0.001% 程度で、Browser 監視を入れる前の値。R9.4 の 50% に対して十分に余裕がある。
- **処理時間**：
  - 本番の保存は 739〜782ms で、その大半が `db insert chats`（730〜777ms、1 件だけ 92ms）。
  - ワーカーが毎回新しく起動するので、PostgREST への接続を毎回確立するコストの可能性がある。
  - **DB スパンがなければ見えなかった情報**で、Phase 5 の DB 監視や性能改善の手がかりになる。
- **計測によるコスト**：自前のトレーサーは依存がないため、読み込みの増加は本番 Spike の「`@opentelemetry/api` だけ」（+23ms）以下と見込む。**本番での直接の比較はしていない**。Trial_Period の中で、計測の有効・無効（`NEW_RELIC_LICENSE_KEY` の有無）を切り替えて比べる。

**マイルストーンの判定：合格**

- 「送信操作 1 件を最後まで追える」は、**バックエンドの区間**（save-chat → DB → triage → okiraku-api → AI の呼び出し）について本番で達成した。
- Browser の区間（Task 3）はまだなので、「操作 → 通信」の部分は未達成。
- データ量は無料枠に対して十分に小さく、送信経路の障害（このあと Phase 3 のアラートで監視する）も見える状態になった。
- 次に進む順番の案：
  1. Phase 3（アラートと PagerDuty）
