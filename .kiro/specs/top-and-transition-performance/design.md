...
- 480 KB 全体の `preload` は外す。critical path で JS と帯域を奪っていた。
  ただし **UI テキスト用サブセット (89 KB) だけは preload する**。これを外すと swap の
  タイミングで文字幅が変わりレイアウトシフトになる（実測で CLS 0.023 → 0、FCP 2.0 → 1.5 s）。
